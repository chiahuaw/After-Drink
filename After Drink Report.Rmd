...
原理很簡單，先用 OpenStreetMap 取得餐廳坐標，用 QGIS 建立金門隨機點坐標。接著利用餐廳的坐標及散布在全金門的隨機點坐標，隨機取兩點出來做為起、終點，然後用 Google Maps API 進行導航。如此一來，我們就取得了 `r length(taxi)` 條導航路徑資料。從中取出直線距離(d)、行駛距離(m)和行駛時間(t)來計算計程車費。
...
做為替代，這裡使用常態分配做為模型去取得。如果有實際上的市場調查，概估就會更接近真值。
...
taxi.trip：從 taxi 中取出的兩點間直線距離(d)、行駛距離(m)和行駛時間(t)，再計算出計程車費(fee)和夜間加姝後的計程車費(nigthfee)

taxi.est：365天的計程車費估計。cars是車次、taxiFee是車資。

taxi.est.y：10000筆的全年計程車費估計。cars是全年累計車次、taxiFee是全年累計車資。建立方式在原始碼裡有。基本上是先隨機取得365筆單日車次，再依車次數取得隨機路徑，計算總計費率，然後重覆1萬次。
